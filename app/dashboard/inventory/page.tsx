...
export default function InventoryPage() {
  ...
  const { ref: quickScanRef } = useZxing({
    paused: !isQuickScanning,
    constraints: { video: { facingMode: "environment" } },
    async onDecodeResult(result) {
      const scannedCode = result.getText();
      setIsQuickScanning(false); // Apagamos la cámara de inmediato

      try {
        // 1. Buscamos si el producto existe por su SKU
        const q = query(
        ...
        if (!querySnapshot.empty) {
          // 2. EL PRODUCTO EXISTE: Aumentamos stock en 1
          const productDoc = querySnapshot.docs[0];
          const productRef = doc(db, "products", productDoc.id);

          await updateDoc(productRef, {
            stock: increment(1), // Función nativa de Firebase para sumar sin necesidad de leer primero el valor
          });
          ...
          Swal.fire({
            ...
            position: "top-end",
            icon: "success",
            title: `+1 Stock añadido a: ${productDoc.data().name}`,
          ...
          // 3. EL PRODUCTO NO EXISTE: Mostramos la advertencia
          Swal.fire({
            icon: "warning",
            title: "No encontrado",
            text: "Producto no registrado, realiza un registro manual.",
            background: "#1f2937",
            color: "#fff",
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {/* NUEVO: Botón de Escaneo Rápido de Stock */}
        <button
        ...
        >
          {isQuickScanning ? <CameraOff size={20} /> : <Barcode size={20} />}
          {isQuickScanning ? "Cancelar Escaneo" : "Ingreso Rápido (+1)"}
        </button>

        {/* Visor de la cámara flotante o colapsable */}
        <div
          className={`overflow-hidden rounded-lg border border-slate-700 bg-black transition-all max-w-sm ${
            isQuickScanning
              ? "h-64 opacity-100 mb-6"
              : "h-0 opacity-0 border-none m-0"
          }`}
        >
          <div className="relative w-full h-full">
            <video ref={quickScanRef} className="w-full h-full object-cover" />
            {isQuickScanning && (
              <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center bg-black/20"></div>
            )}
          </div>
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-2">
                        <span
                          className={`text-sm font-bold ${product.stock <= product.minStock ? "text-red-400" : "text-white"}`}
                        >
                          {product.stock}
                        </span>
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1 bg-slate-800/50 px-2 py-0.5 rounded-md border border-slate-700/50">
                      <span
                        className={`text-xs font-bold ${product.stock <= product.minStock ? "text-red-400" : "text-slate-300"}`}
                      >
                        {product.stock} pz
                      </span>
                      {product.stock <= product.minStock && (
