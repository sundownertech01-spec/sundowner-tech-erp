...
import { Plus, Search, Edit, Trash2, Loader2, Wrench, MapPin } from "lucide-react";
...
interface Tool {
  ...
  quantity: number;
  condition: string;
  location: string; // NUEVO: Agregamos la ubicación a la interfaz
}
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="bg-slate-900 p-3 md:p-4 rounded-xl border border-slate-800">
        <div className="relative w-full">
          ...
          {/* NUEVO: Buscador permite buscar por caja */}
          <input type="text" placeholder="Buscar herramienta o ubicación (ej. Caja 1)..." className="block w-full pl-10 pr-3 py-2 border border-slate-700 rounded-lg bg-slate-800 text-slate-300 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-colors" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-xl overflow-hidden">
        ...
        {!isLoading && filteredTools.length === 0 && (<div className="p-12 text-center text-slate-400">No hay herramientas registradas aún.</div>)}
                  ...
                  <tr key={tool.id} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                        tool.condition === 'Mala' ? 'bg-red-500/10 text-red-400' : 
                        tool.condition === 'Regular' ? 'bg-orange-500/10 text-orange-400' : 
                ...
                <div className="flex justify-between items-start gap-2">
                  <div className="flex-1">
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-2">
                      <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-md bg-slate-800 text-slate-300 border border-slate-700">
                        {tool.category}
                      </span>
                      {/* NUEVA ETIQUETA DE UBICACIÓN PARA MÓVIL */}
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-md bg-indigo-500/10 text-indigo-300 border border-indigo-500/20">
                        <MapPin size={10} /> {tool.location || "Sin asignar"}
                      </span>
                    </div>
                  </div>
